...
public class CameAndWentProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch(uriMatcher.match(uri)){
            case KEY_MONTHS:
                c = sdb.query(TIME_TABLE, new String[]{ID, MONTH_OF_YEAR}, null, null,
                              MONTH_OF_YEAR, null, sortOrder, null);
                c.setNotificationUri(getContext().getContentResolver(), URI_MONTHS);
                return c;
            case KEY_WEEKS:
                c = sdb.query(TIME_TABLE, new String[]{ID, WEEK_OF_YEAR}, null, null,
                              WEEK_OF_YEAR, null, sortOrder, null);
                c.setNotificationUri(getContext().getContentResolver(), URI_WEEKS);
                return c;
            case KEY_DURATIONS_PER_DAY:
                c = sdb.query(VIEW_DURATIONS_PER_DAY, projection, selection, selectionArgs, DATE,
                              null, sortOrder, null);
                c.setNotificationUri(getContext().getContentResolver(), URI_DURATIONS_PER_DAY);
                return c;
            case KEY_GET_LOG_ENTRY_FOR_EDIT:
                c = sdb.query(TABLE_LOG_NAME, projection, selection, selectionArgs, null, null,
                              sortOrder, "1");
                c.setNotificationUri(getContext().getContentResolver(), URI_LOG_ENTRIES);
                return c;
            case KEY_LOG_ENTRIES:
                c = sdb.query(VIEW_LOG, projection, selection, selectionArgs, null, null,
                              sortOrder);
                c.setNotificationUri(getContext().getContentResolver(), URI_LOG_ENTRIES);
                return c;
            case KEY_MONTHLY_SUMMARY:
                c = sdb.query(VIEW_TIME_TABLE_DURATIONS, new String[]{ID, DATE, WEEK_OF_YEAR,
                        "SUM(" + DURATION + ") AS " + DURATION}, selection, selectionArgs,
                              WEEK_OF_YEAR, null, sortOrder, null);
                c.setNotificationUri(getContext().getContentResolver(), URI_MONTHLY_SUMMARY);
                return c;
            case KEY_DURATIONS:
                c = sdb.query(VIEW_TIME_TABLE_DURATIONS, projection, selection, selectionArgs,
                              null, null, sortOrder, null);
                c.setNotificationUri(getContext().getContentResolver(), URI_DURATIONS);
                return c;
            case KEY_TAGS:
                c = sdb.query(TABLE_TAGS_NAME, projection, selection, selectionArgs, null, null,
                              sortOrder, null);
    ...
    public void clean(){
        ...
        Cursor c = query(CameAndWentProvider.URI_LOG_ENTRIES, null, String.format("%s>?", WENT),
                         new String[]{"0"}, null);
        for(c.moveToFirst(); !c.isAfterLast(); c.moveToNext())
            if(c.getLong(c.getColumnIndex(CameAndWentProvider.WENT)) - c.getLong(c.getColumnIndex
                    (CameAndWentProvider.CAME)) < DateTimeConstants.MILLIS_PER_MINUTE)
                sdb.delete(TABLE_LOG_NAME, String.format("%s=?", ID), new String[]{String.valueOf
    ...
    public void seed(SQLiteDatabase sdb){
        ...
        for(DateTime dt = getSeedDateTime(); dt.getDayOfYear() <= dtNow.getDayOfYear(); dt = dt
                .withFieldAdded(DurationFieldType.days(), 1)){
            if(dt.getDayOfWeek() == DateTimeConstants.SATURDAY || dt.getDayOfWeek() ==
            ...
            boolean useSnapup = PreferenceManager.getDefaultSharedPreferences(getContext())
                                                 .getBoolean("use_snapup", false);
    ...
    private class DatabaseOpenHelper extends SQLiteOpenHelper{
        ...
        private String getDurationCalculation() {
            //TODO: snapup on project basis instead of daily??
